...
ALL_PARAMETERS = {
    ...
    'G_PARAM_EMU_BALANCE': 10.0,
    # 模拟模式的账号余币，实盘模式会忽略此参数
    'G_PARAM_EMU_STOCKS': 10.0,
...
class MyExchange(object):
    def __init__(self, botvs_exchange):
        ...
        self.trdfee = TRDFEE_DICT.get(self.name, DEFAULT_TRDFEE)
    ...
    def buy(self, price, amount):
        ...
        if is_emulate_mode():
            ...
            if self.account['Balance'] < cost:
                LogError('买入失败：余额 %.*f < %.*f'
                         % (prec, self.account['Balance'], prec, cost))
                return None
            self.account_diffs.append({
                'Balance': -cost,
                'FrozenBalance': 0.0,
                'Stocks': amount,
                'FrozenStocks': 0.0,
    ...
    def sell(self, price, amount):
        ...
        if is_emulate_mode():
            if self.account['Stocks'] < amount:
                LogError('卖出失败：余币 %.*f < %.*f'
                         % (prec, self.account['Stocks'], prec, amount))
                return None
    ...
    def get_account(self):
        if is_emulate_mode():
            # 重放变化
            for i in self.account_diffs:
                for k in ('Balance', 'FrozenBalance', 'Stocks', 'FrozenStocks'):
                    self.account[k] += i[k]
            # 我没有找到同时兼容py2和py3的清空数组的优雅方法...
            self.account_diffs = []
            return self.account
    ...
    def tune_delta(self, myex_b):
        ...
        if delta_magic['times'] >= 20:
            ...
            #diff_avg = self.delta_magic[b.name]['diffs'] / 20.0
            ...
            if get_loop_count() % 1000 == 0:
                LogDebug('最近学习到的差价为 %.*f，更新 delta (%s - %s)：%.*f -> %.*f'
                         % (prec, diff_avg,
                            a.name, b.name,
                            prec, prev_delta,
...
def refresh_status(myex_list, all_curr_fund, all_init_fund, op_delay_ms):
    ...
    draw_table(table_summary, table0)
